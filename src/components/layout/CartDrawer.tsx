...
export default function CartDrawer() {
  ...
  return (
    <div 
      className={`fixed inset-0 w-full h-full bg-[#f8f8f8] z-[100] transform transition-transform duration-300 ease-[cubic-bezier(0.8,0,0.2,1)] flex flex-col ${
        isCartOpen ? 'translate-x-0' : 'translate-x-full'
      }`}
    >
      {/* Header */}
      <div className="flex items-center justify-between p-4 md:px-6 border-b-2 border-black bg-white shrink-0">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 flex flex-col gap-4">
        {items.length === 0 ? (
          ...
          <div className="max-w-[700px] w-full mx-auto flex flex-col gap-4">
            {items.map((item) => (
              <div key={item.id} className="relative flex gap-3 p-3.5 md:p-4 border-2 border-black rounded-[20px] bg-white shadow-[4px_4px_0px_#000]">
                ...
                <div className="flex-1 flex flex-col justify-between py-0.5 pr-8">
                  ...
                  <div className="flex items-center w-fit border-2 border-black rounded-full mt-2 overflow-hidden bg-white">
                    <button 
      ...
      {/* Footer */}
      {items.length > 0 && (
        <div className="px-4 py-2 md:px-8 md:py-6 border-t-2 border-black bg-white flex flex-col shrink-0">
